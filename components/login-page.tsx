...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // onAuthStateChanged in page.tsx handles the rest (loadFromCloud, setUser, navigation)
    } catch (err: any) {
      const code = err?.code || '';
      ...
      <div className="w-full max-w-4xl flex rounded-2xl overflow-hidden border border-white/8 shadow-2xl">
        ...
        <div className="hidden md:flex flex-col w-5/12 p-10 relative overflow-hidden bg-[#0d1018]">
          ...
          <div className="relative z-10 space-y-5 pb-52">
            <FeatureItem icon={<BarChart3 className="w-5 h-5 text-blue-400" />} title="Track Progress" desc="Monitor your syllabus completion and stay on track." />
            <FeatureItem icon={<Target className="w-5 h-5 text-purple-400" />} title="Stay Focused" desc="Keep your focus on what matters the most." />
            <FeatureItem icon={<TrendingUp className="w-5 h-5 text-green-400" />} title="Achieve Goals" desc="Analyze your performance and achieve your target." />
          </div>
        </div>
        ...
        <div className="w-full md:w-7/12 p-10 bg-[#0f1219]">
          <div className="max-w-sm mx-auto">
            ...
            <p className="text-gray-400 text-sm mb-8">
              {mode === 'login' ? 'Login to continue your journey' : 'Sign up to start tracking your prep'}
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {mode === 'login' && (
                <div className="flex justify-end -mt-2">
                  <button type="button" onClick={handleForgotPassword} className="text-sm text-purple-400 hover:text-purple-300 transition-colors">
                    Forgot Password?
                  </button>
                </div>
              )}
            ...
            <p className="text-center text-sm text-gray-500 mt-8">
              ...
              <button
                onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); clearError(); setResetSent(false); }}
            ...
            <p className="text-center text-xs text-gray-600 mt-4">Data is stored securely in your account.</p>
          </div>
        </div>
      </div>
    </div>
